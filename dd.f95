...
contains
    ...
    ! A : matrice retournée
    subroutine build_A_N(t_courant, dt, m, psi, psi_l, psi_r, B, A)
        ! paramètres
        real(rp) :: t_courant
        real(rp) :: dt
        type(Mesh), intent(in) :: m
    ...
    ! B : fonction schéma parmi B1, B2, B3
    ! A : matrice retournée
    subroutine build_A_P(t_courant, dt, m, psi, psi_l, psi_r, B, A)
        ! paramètres
        real(rp) :: t_courant
        real(rp) :: dt
        type(Mesh), intent(in) :: m
    ...
    subroutine build_b_N(t_courant, dt, m, psi, n, psi_l, psi_r, n_l, n_r, B, b_N)
        ! paramètres
        real(rp), intent(in) :: t_courant
        real(rp), intent(in) :: dt
        type(Mesh), intent(in) :: m
        real(rp), dimension(m%l), intent(in) :: psi, n
        real(rp), external :: psi_l, psi_r, n_l, n_r
        real(rp), external :: B
        ...
        b_N(1) = b_N(1) + B(psi_l(t_courant)-psi(1)) / m%h2(1)
        b_N(m%l) = b_N(m%l) + B(psi_r(t_courant)-psi(m%l)) / m%h2(m%l+1)
    end subroutine



    ! -------------------------------------------------------------------------------------------------------
    ! Matrice b_P (b semi implicite)
    ! -------------------------------------------------------------------------------------------------------
    ! t_courant : temps donné
    ! dt : pas de temps
    ! m : maillage d'espace
    ...
    ! B : fonction schéma parmi B1, B2, B3
    ! b_N : matrice retournée
    subroutine build_b_P(t_courant, dt, m, psi, p, psi_l, psi_r, p_l, p_r, B, b_P)
        ! paramètres
        real(rp), intent(in) :: t_courant
        real(rp), intent(in) :: dt
        type(Mesh), intent(in) :: m
        real(rp), dimension(m%l), intent(in) :: psi, p
        real(rp), external :: psi_l, psi_r, p_l, p_r
        real(rp), external :: B
        real(rp), dimension(m%l), intent(out) :: b_P
        ...
        b_P(1) = b_P(1) + B(-psi_l(t_courant)+psi(1)) / m%h2(1)
        b_P(m%l) = b_P(m%l) + B(-psi_r(t_courant)+psi(m%l)) / m%h2(m%l+1)
    end subroutine
    ...
    subroutine vf_semi_implicite_dd(t, m, B, c, n0, p0, n_l, n_r, p_l, p_r, psi_l, psi_r, n, p, psi)
        ...
        integer :: i
        real(rp), dimension(m%l) :: n_suiv, p_suiv, psi_suiv, ci
        real(rp), dimension(m%l, m%l) :: A_N, A_P
        real(rp), dimension(m%l) :: b_N, b_P
        ...
        do i = 1, size(t)
            ! calcul itéré n suivant
            call build_A_N(t(i), dt, m, psi, psi_l, psi_r, B, A_N)
            call build_b_N(t(i), dt, m, psi, n, psi_l, psi_r, n_l, n_r, B, b_N)
            call linSolve("plu", m%l, A_N, b_N, n_suiv)

            ! calcul itéré p suivant
            call build_A_P(t(i), dt, m, psi, psi_l, psi_r, B, A_P)
            call build_b_P(t(i), dt, m, psi, p, psi_l, psi_r, p_l, p_r, B, b_P)
            call linSolve("plu", m%l, A_P, b_P, p_suiv)
